...
from .database import init_db, get_engine, save_daily_metrics, save_signals
...
@app.command()
def run(ticker:str = typer.Option(..., "--ticker", "-t", help="Stock ticker symbol"), output: str = 'output.json', config: str = None):
    ...
    df = process_data(raw, min_sma_days=cfg.get('data_settings', {}).get('min_trading_days_for_sma', 200))
